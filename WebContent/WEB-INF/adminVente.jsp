...
<body>
	...
	<table>
		...
		<%
			if(request.getAttribute("venteListe") != null)
			{
				for(Vente vente : (ArrayList<Vente>)request.getAttribute("venteListe"))
				{%>
					<tr>
						<td><a href="adminControler?idVente=<%= vente.getIdVente() %>"><%= vente.getIdVente() %></a></td>
						<td><%= vente.getDate() %></td>
						<td><%= vente.getPseudo() %></td>
						<td><%= vente.getMail() %></td>
					</tr>
				<%}
			}
