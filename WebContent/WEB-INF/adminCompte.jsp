...
<body>
	...
	<table>
		...
		<%
			if(request.getAttribute("compteListe") != null)
			{
				for(Compte compte : (ArrayList<Compte>)request.getAttribute("compteListe"))
				{%>
					<tr>
						<td><%= compte.getPseudo() %></td>
						<td><%= compte.getNom() %></td>
						<td><%= compte.getPrenom() %></td>
						<td><%= compte.getEmail() %></td>
						<td><%= compte.isAdmin() %></td>
						<td><form action="adminControler" method="post"><input type="submit" value="+"><input type="hidden" name="addAdmin" value="<%= compte.getIdCompte() %>"></form></td>
						<td><form action="adminControler" method="post"><input type="submit" value="-"><input type="hidden" name="suppAdmin" value="<%= compte.getIdCompte() %>"></form></td>
					</tr>
				<%}
			}
		%>
