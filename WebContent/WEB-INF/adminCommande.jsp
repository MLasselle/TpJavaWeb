<%@page import="entitee.Commande"%>
<%@page import="entitee.Vente"%>
...
<link rel="stylesheet" href="css/adminCommande.css">
...
<body>
	...
	<h1>Liste de la commande</h1>
	<table id="commandeListe">
		<tr>
			<th>idVente</th>
			<th>date</th>
			<th>idProduit</th>
		...
		<%
			double total = 0;
			if(request.getAttribute("commandeListe") != null)
			{
				for(Commande cmd : (ArrayList<Commande>)request.getAttribute("commandeListe"))
				{%>
					<tr>
						<td><%= cmd.getIdVente() %></td>
						<td><%= cmd.getDate() %></td>
						<td><%= cmd.getIdProduit() %></td>
						<td><img src="imageProduit/<%= cmd.getIdProduit() %>/1.jpg"></td>
						<td><%= cmd.getTitre() %></td>
						<td><%= cmd.getQte() %></td>
						<td><%= cmd.getPrix() * cmd.getQte() %></td>
						<%
							total += cmd.getPrix() * cmd.getQte();
						%>
					</tr>
				<%}
			}
	...
	<p>Le total de la commande est de: <%= total %>$<p>
	</section>
	<footer>
